...
X_norm = pd.DataFrame(quantile_transform(X, n_quantiles=7, random_state=0))
...
pca = PCA(n_components=38, random_state=10)
...
plt.title('PCA of NBA Stats dataset')

#################################################
#ICA feature transformation
    
ica = FastICA(n_components=38, random_state=10)
...
for color, i in zip(colors, [4,8]):
    plt.scatter(X_r[y == i, 0], X_r[y == i, 1], color=color, alpha=.8, lw=lw, label=i)
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('ICA of NBA Stats dataset')

#################################################
#Random Projection feature transformation

rca = GaussianRandomProjection(n_components=38, random_state=10)
...
for color, i in zip(colors, [4,8]):
    plt.scatter(X_r[y == i, 0], X_r[y == i, 1], color=color, alpha=.8, lw=lw, label=i)
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('Random Projection of NBA Stats dataset')
...
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('Chi square feature selection of NBA Stats dataset')
plt.show()
...
colors = ["b", "g", "r", "c", "m", "y", "k"]
transform_features=['PCA', 'ICA', 'RCA', 'K-Best']
# Make the plot
for i in range(len(transform_features)):
    plt.bar(transform_features[i], running_time[i], color=colors[i], label=transform_features[i])
plt.xticks(transform_features, ('PCA', 'ICA', 'RCA', 'K-Best'))
...
X_df[38] = y_kmeans
plot_learning_curve(clf, "MLP using selected 5 features with clusters new feature", X_df, y, ylim=[0, 1])
running_time.append(time.time()-start)
